...
- 每次 CLI 调用独立连接，无后台连接池。多步操作需要同一上下文时，合并成一次命令或脚本，可减少重复握手。不会自动应用 TOML 中的默认目录，也不会在不同 exec 之间保留 cd 或变量。
...
- `doctor` 只检查本地配置；`exec --debug` 输出连接与执行阶段耗时。每次调用结束回收自己的 SSH 连接，不批量终止其他 ssh/node 进程。
